...
macro_rules! send_message {
    ($self:ident, $t:ty, $data:expr) => { $self.issue_system_async::<MessageWrapper<$t>>(MessageWrapper { 0: $data }); };
}

/// Helper macro to subscribe to a given message type
macro_rules! subscribe_to {
    ($t:ty, $self:ident, $ctx:expr) => { $self.subscribe_sync::<BrokerType, MessageWrapper<$t>>($ctx) };
}
